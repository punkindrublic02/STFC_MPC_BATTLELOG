...
export const Buffs = ({ parsedData, input, data }: BuffsProps) => {
  ...
  allBuffs.forEach((buff) => {
    const assetKind =
      buff.data?.type === "research" ||
      buff.data?.type === "building" ||
      buff.data?.type === "forbidden_tech" ||
      buff.data?.type === "officer" ||
      buff.data?.type === "consumable"
        ? buff.data.type
        : undefined;
    const assetId = buff.data?.type === "consumable" ? buff.buff_id : buff.activator_id;
    ...
    tableData.push([
      ...
      assetKind ? (
        <CombatAssetLabel
          ...
          kind={assetKind}
          id={assetId}
          label={buff.activatorDisplayName}
          secondary={assetKind === "forbidden_tech" ? "Forbidden Tech" : undefined}
          size={28}
        />
