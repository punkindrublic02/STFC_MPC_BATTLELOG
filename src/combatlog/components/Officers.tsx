...
function officerDescription(ship: CombatLogShip, index: number, data: GameData): CombatLogCell {
  ...
  if (!!officer) {
    ...
    return (
      <CombatAssetLabel
        ...
        kind="officer"
        id={officer.id}
        label={officerDetail.officerName}
        secondary={`T${officer.rank} L${officer.level}`}
        captain={index === 0}
      />
    );
  } else {
...
export const Officers = ({ parsedData, input, data, raw_json }: OfficersProps) => {
  ...
  tableData.push([
    ...
    ...ships.map((s) => {
      ...
      return (
        <CombatAssetLabel
          key={`${s.shipId}:${hullId}:header`}
          data={data}
          kind="ship"
          id={hullId}
          label={getShipName(s, input, data)}
